...
do
    ...
    tee_muxer_output+="[f=fifo:fifo_format=flv:queue_size=120:drop_pkts_on_overflow=1:attempt_recovery=1:recovery_wait_time=10:max_recovery_attempts=5:restart_with_keyframe=1]${platform}"
    i=$((i+1))
done

while true :
do
	ffmpeg -threads "${THREAD_COUNT}" -loglevel debug \
